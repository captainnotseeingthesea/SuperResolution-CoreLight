...
# 使用Improved NEDI算法完成upscale操作
# img：要进行upscale的图像
# m：NEDI算法的window大小
# method：完成边缘检测的算法
def INEDI_upscale(img, m, method, iterate):
                ...
                for k in range(c):
                    ...
                    imgo[2 * i + 1][2 * j + 1][k] = np.matmul(
                            [imgo[2 * i][2 * j][k], imgo[2 * i + 2][2 * j][k], imgo[2 * i + 2][2 * j + 2][k],
                             imgo[2 * i][2 * j + 2][k]], weight_A[i//(2**iterate)][j//(2**iterate)][k])
            ...
            if edge[i][j] != 0:
                for k in range(c):
                    if True:
                        ...
                        #      imgo[2 * i + 1][2 * j + 1][k]], weight_B[i//(2**iterate)][j//(2**iterate)][k])
                        # imgo[2 * i][2 * j + 1][k] = np.matmul(
                        #     [imgo[2 * i - 1][2 * j + 1][k], imgo[2 * i][2 * j][k], imgo[2 * i + 1][2 * j + 1][k],
                        #      imgo[2 * i][2 * j + 2][k]], weight_B[i//(2**iterate)][j//(2**iterate)][k])
                        imgo[2 * i + 1][2 * j][k] = np.matmul(
                            [imgo[2 * i][2 * j][k], imgo[2 * i + 1][2 * j - 1][k], imgo[2 * i + 2][2 * j][k],
...
def EDI_predict(img, m, s, method="canny"):
    ...
    if s <= 0:
        ...
        for i in range(n):
            img = INEDI_upscale(img, m, method, i)
...
if __name__ == '__main__':
    ...
    # 使用INEDI进行插值
    size = 4
    w, h, c = downscale_img.shape
    weight_A = np.zeros((w, h, c, 4))
    weight_B = np.zeros((w, h, c, 4))
    inedi_img = EDI_predict(downscale_img, size, factor, "none")
    cv2.imwrite("inedi_lenna.jpg", inedi_img)

    # 输出各种upscaling算法的pnsr和ssim
    src_img_path = img_path
    upscale_img_paths = ["linear_lenna.jpg", "bicubic_lenna.jpg", "nedi_lenna.jpg", "inedi_lenna.jpg"]
