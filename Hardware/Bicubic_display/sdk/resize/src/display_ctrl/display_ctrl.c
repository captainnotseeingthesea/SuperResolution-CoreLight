...
**
*/
int DisplayInitialize(DisplayCtrl *dispPtr, XAxiVdma *vdma, u16 vtcId, u32 dynClkAddr, u8 *framePtr[DISPLAY_NUM_FRAMES], u32 stride, u32 img_width, u32 img_height)
{
	int Status;
	int i;
	...
	dispPtr->shift_stride = img_width / 2;
	dispPtr->img_width = img_width;
	dispPtr->img_height = img_height;
...
/***	DisplayChangePos(DisplayCtrl *dispPtr, u32 direction)
**
**	Parameters:
**		dispPtr - Pointer to the initialized DisplayCtrl struct
**		direction - direction of shifting on a large image
...
**
**	Description:
**		Changes the position of the image currently being displayed.
**
*/

int DisplayChangePos(DisplayCtrl *dispPtr, u32 direction)
{
	int Status;
	int i;
	int shift_x = 0, shift_y = 0;
	int max_right = dispPtr->img_width - dispPtr->vMode.width;
	int max_down = dispPtr->img_height - dispPtr->vMode.height;
	
	switch (direction)
	{
	case 0:
		shift_y = ((dispPtr->pos_y - dispPtr->shift_stride) < 0) ? (0 - dispPtr->pos_y) : (0 - dispPtr->shift_stride);
		dispPtr->pos_y = ((dispPtr->pos_y - dispPtr->shift_stride) < 0) ? 0 : (dispPtr->pos_y - dispPtr->shift_stride);
		break;
	case 1:
		shift_y = ((dispPtr->pos_y + dispPtr->shift_stride) > max_down) ? (max_down - dispPtr->pos_y) : dispPtr->shift_stride;
		dispPtr->pos_y = ((dispPtr->pos_y + dispPtr->shift_stride) > max_down) ? max_down : (dispPtr->pos_y + dispPtr->shift_stride);
		break;
	case 2:
		shift_x = ((dispPtr->pos_x - dispPtr->shift_stride) < 0) ? (0 - dispPtr->pos_x) : (0 - dispPtr->shift_stride);
		dispPtr->pos_x = ((dispPtr->pos_x - dispPtr->shift_stride) < 0) ? 0 : (dispPtr->pos_x - dispPtr->shift_stride);
		break;
	case 3:
		shift_x = ((dispPtr->pos_x + dispPtr->shift_stride) > max_right) ? (max_right - dispPtr->pos_x) : dispPtr->shift_stride;
		dispPtr->pos_x = ((dispPtr->pos_x + dispPtr->shift_stride) > max_right) ? max_right : (dispPtr->pos_x + dispPtr->shift_stride);
		break;
	default:
		break;
	}
	...
	if (dispPtr->state == DISPLAY_RUNNING)
	{
		...
		{
			dispPtr->framePtr[i] = dispPtr->framePtr[i] + (shift_y * dispPtr->stride) + shift_x * 3;
		}
		...
		if (Status != XST_SUCCESS)
		{
			xdbg_printf(XDBG_DEBUG_GENERAL, "Cannot change the position of the frame %d\r\n", Status);
			return XST_FAILURE;
		}
